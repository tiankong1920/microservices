...
```bash
# 构建镜像
docker build -t inventory/order-service:3.0.0 .

# 运行容器
docker run -p 8084:8084 inventory/order-service:3.0.0
```

## API文档

服务启动后访问: http://localhost:8084/swagger-ui.html
...
主要配置文件位于 `config-repo/order-service/`
