...
```bash
# 构建镜像
docker build -t inventory/inventory-service:3.0.0 .

# 运行容器
docker run -p 8083:8083 inventory/inventory-service:3.0.0
```

## API文档

服务启动后访问: http://localhost:8083/swagger-ui.html
...
主要配置文件位于 `config-repo/inventory-service/`
