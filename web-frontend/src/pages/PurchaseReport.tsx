...
import { ShoppingCart as ShoppingCartIcon, TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, Download as DownloadIcon, Print as PrintIcon } from '@mui/icons-material';
import { reportApi } from '../services/api';
import ExportService from '../services/exportService';
import Toast from '../components/Toast';
import type { ApiResponse, PurchaseReport as PurchaseReportType } from '../types';

const PurchaseReport: React.FC = () => {
  const [report, setReport] = useState<PurchaseReportType | null>(null);
  ...
  const loadReport = async (): Promise<void> => {
    ...
    setError(null);
    try {
      const response: ApiResponse<PurchaseReportType> = await reportApi.getPurchaseReport({ startDate, endDate });
      setReport(response.data);
    } catch (err) {
      console.error('Failed to load purchase report:', err);
      setError('加载采购报表失败');
      setToast({ open: true, message: '加载采购报表失败', severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportPDF = (): void => {
    ...
    const fields = [
      ...
      { key: 'totalPurchases', label: '采购订单数' },
      { key: 'totalAmount', label: '采购金额', format: (v: number) => `¥${v || 0}` },
      { key: 'averageOrderValue', label: '平均订单金额', format: (v: number) => `¥${v || 0}` },
    ];
    ExportService.exportToPDF([report], { filename: 'purchase-report', fields });
