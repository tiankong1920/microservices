...
import type { Supplier, SupplierFormData, FormErrors } from '../types';
...
const SupplierEdit: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.supplierName.trim()) {
      newErrors.supplierName = '供应商名称不能为空';
    }
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      if (isEditMode && id) {
        await supplierApi.updateSupplier(Number(id), formData);
      ...
      showError(isEditMode ? '更新供应商失败' : '创建供应商失败');
    } finally {
      setSubmitting(false);
    }
  };
